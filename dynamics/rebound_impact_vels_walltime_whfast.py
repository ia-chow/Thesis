...
def my_merge(sim_pointer, collided_particles_index):
    ...
    sim.save_to_file(f'sim_particles_{i.hash}{j.hash}_col_walltime_fast.bin')
...
sim.save_to_file('rebound_archive_walltime_fast.bin', walltime=7200, delete_file=True)  # every two hours

# integrate until there is a collision and keep recording positions and velocities
times = np.linspace(0., 1.e8, int(1e6))  # integrate
...
# save
np.save('earth_cart_positions_whfast', earth_cart_positions)
np.save('earth_cart_vels_whfast', earth_cart_vels)
np.save('ast_cart_positions_whfast', ast_cart_positions)
np.save('ast_cart_vels_whfast', ast_cart_vels)