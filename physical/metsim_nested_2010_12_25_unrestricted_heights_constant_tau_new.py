...
logl_scale_factor = 1.#e24  # factor to scale the log likelihood by, THIS IS CONSTANT
...
# EVENT
event_path = '../usg_metsim_files/2010-12-25/usg_input_dec_2010_trunc' # event path, CHANGE THIS
...
def get_lc_cost_function(flattened_free_params, metsim_obj, pso=False):
    ...
    *frag_grain_maxs: n grain maxs
    ...
    sigma = np.exp(flattened_free_params[0])  # 0 is in sigma
...
print('sigma mean, median, min, max:', ln_sigma_mean, ln_sigma_median, ln_sigma_min, ln_sigma_max)

# print initial guess log likelihood
# CHANGE THIS
initial_guess = np.load('initial_guesses/initial_guess_2010_12_25_TEST_heights.npy') # initial pre-saved guess
...
frag_erosion_coeffs = tuple([(0.01 * 1.e-6, 10.0 * 1.e-6) for entry in 
...
frag_grain_mins = tuple((1.e-4, 1.e3) for _ in range(fragmentation_count))
# Frag grain maxs
frag_grain_maxs = tuple((1.e-3, 1.e4) for _ in range(fragmentation_count))
# Frag heights
...
hard_bounds = np.array(((ln_sigma_min - 10., ln_sigma_max + 10.), (1.e4, 1.e8), ) + result)  # log sigma and mass

# CHANGE THIS DEPENDING ON EVENT

# generic unrestricted bounds
bounds = hard_bounds

# # generate less restrictive bounds - 2004-09-03
# bounds = np.array(hard_bounds)
...
# CHANGE THIS
height_bounds = 750.  # permissible values plus or minus the starting fragmentation height
...
bounds[1] = (1.e4, 1.e8)
# frag msses
bounds[2:fragmentation_count + 2] = np.repeat([(0., 100.)], fragmentation_count, axis=0)
# heights
...
# Run NS:
import dynesty
filename = './dynesty_saves/2010_12_25_unrestricted_heights_constant_tau_new.save'  # CHANGE THIS
