...
# EVENT
event_path = '../usg_metsim_files/2004-09-03/usg_input_sep_2004'  # event path, CHANGE THIS
...
# CHANGE THIS
initial_guess = np.load('initial_guess_2004_09_03_TEST.npy')  # initial pre-saved guess
...
frag_grain_mins = tuple((1e-3, 1e2) for _ in range(fragmentation_count))
# Frag grain maxs
frag_grain_maxs = tuple((1e-2, 1e3) for _ in range(fragmentation_count))
...
cov_factor = 1.e-0  # CHANGE THIS DEPENDING ON THE EVENT
...
def gaussian_prior(cube, mu=mu, cov=cov):
    """
...
def prior(cube, tup_bounds=bounds, n_frags=fragmentation_count, er_frags=len(ER_FRAG_INDICES)):
    ...
    # note that grain max depends on grain min
...
# Run NS:
import dynesty
filename = '2004-09-03_gaussian.save'  # CHANGE THIS

with dynesty.pool.Pool(32, log_likelihood, gaussian_prior, 
                       logl_args=(metsim_obj, 10), 
                       ptform_args=(mu, cov)) as pool:
    # NEW RUN
