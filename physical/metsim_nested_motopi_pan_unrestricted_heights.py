...
# EVENT
event_path = '../usg_metsim_files/non_decameter/motopi_pan/usg_input_motopi_pan' # event path, CHANGE THIS
...
# CHANGE THIS
initial_guess = np.load('initial_guesses/initial_guess_motopi_pan_TEST_heights.npy') # initial pre-saved guess
...
frag_grain_mins = tuple((1.e-5, 1.e2) for _ in range(fragmentation_count))
# Frag grain maxs
frag_grain_maxs = tuple((1.e-4, 1.e3) for _ in range(fragmentation_count))
# Frag heights
...
hard_bounds = np.array(((1.e2, 1.e6), ) + result)
...
# CHANGE THIS
height_bounds = 500.  # permissible values plus or minus the starting fragmentation height
...
def prior(cube, tup_bounds=bounds, n_frags=fragmentation_count, er_frags=len(ER_FRAG_INDICES)):
    ...
    frag_height_bnds = bounds[3 * n_frags + er_frags + 1: 4 * n_frags + er_frags + 1]
    ...
    cube[3 * n_frags + er_frags + 1: 4 * n_frags + er_frags + 1] = cube[3 * n_frags + er_frags + 1: 4 * n_frags + er_frags + 1] * (frag_height_bnds[:, 1] - frag_height_bnds[:, 0]) + frag_height_bnds[:, 0]
...
# Run NS:
import dynesty
filename = './dynesty_saves/motopi_pan_unrestricted_heights.save'  # CHANGE THIS
