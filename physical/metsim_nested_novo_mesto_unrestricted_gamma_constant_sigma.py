...
logl_scale_factor = 1.e22  # factor to scale the log likelihood by, DON'T CHANGE THIS

# Modified version to run with optimization routine (now including single-body fragments):
null=None
class MetSimObj():
    def __init__(self, traj_path, const_json_file, lc_path, fixed_frag_indices, free_frag_indices, er_frag_indices, mass_frag_indices):
        ...
        self.mass_frag_indices = mass_frag_indices
        ...
        # fragments with number/mass (i.e. everything except A)
        mass_frag_mask = np.zeros(num_frags, bool)
        mass_frag_mask[mass_frag_indices] = True
        ...
        for i, frag in enumerate(const_json.get('fragmentation_entries')):
            ...
            if frag['erosion_coeff'] == None:  # this means that the fragment is dust, F or A
        ...
        # print(mass_frag_mask)
        # assign free fixed parameters to object
        self.free_params = [m_init,
                            list(np.array(frag_gammas)[~mass_frag_mask]),  # use mass frag mask for this one
                            list(np.array(frag_masses)[mass_frag_mask]), # use a frag mask for this one!
        ...
        self.fixed_params = [dt, P_0m, h_init, m_kill, v_kill, h_kill, len_kill, rho, rho_grain, sigma, 
                            ...
                            frag_ab_coeffs, 
                            list(np.array(frag_gammas)[mass_frag_mask]), 
                            frag_mis,
                            list(np.array(frag_masses)[~mass_frag_mask]), # and inverse of a frag mask
    ...
    def initializeSimulation(self, all_params):
            ...
            m_init, frag_gammas_free, frag_masses_free, frag_er_coeffs_free, frag_grain_mins_free, frag_grain_maxs_free, frag_heights_free = free_params 
            dt, P_0m, h_init, m_kill, v_kill, h_kill, len_kill, rho, rho_grain, sigma, v_init, shape_factor, gamma, zenith_angle, lum_eff, lum_eff_type, erosion_height_start, erosion_bins_per_10mass, erosion_coeff, erosion_height_change, erosion_coeff_change, erosion_mass_index, erosion_mass_min, erosion_mass_max, erosion_rho_change, erosion_sigma_change, compressive_strength, disruption_erosion_coeff, disruption_mass_grain_ratio, disruption_mass_index, disruption_mass_min_ratio, disruption_mass_max_ratio, frag_types, frag_numbers, frag_ab_coeffs, frag_gammas_fixed, frag_mis, frag_masses_fixed, frag_er_coeffs_fixed, frag_grain_mins_fixed, frag_grain_maxs_fixed, frag_heights_fixed = fixed_params
            # load all the global parameters into the object
            consts = dt, P_0m, h_init, m_kill, v_kill, h_kill, len_kill, rho, rho_grain, m_init, sigma, v_init, shape_factor, gamma, zenith_angle, lum_eff, lum_eff_type, erosion_height_start, erosion_bins_per_10mass, erosion_coeff, erosion_height_change, erosion_coeff_change, erosion_mass_index, erosion_mass_min, erosion_mass_max, erosion_rho_change, erosion_sigma_change, compressive_strength, disruption_erosion_coeff, disruption_mass_grain_ratio, disruption_mass_index, disruption_mass_min_ratio, disruption_mass_max_ratio
            self.loadGlobalParameters(consts)
            # combine free fixed fragmentation parameters
            frag_order = np.argsort(self.fixed_frag_indices + self.free_frag_indices)
            frag_gammas = np.repeat(None, len(frag_order))
            frag_gammas[list(set(self.free_frag_indices) - set(self.mass_frag_indices))] = frag_gammas_free
            frag_masses = np.zeros(len(frag_order))
            frag_masses[self.mass_frag_indices] = frag_masses_free
            frag_er_coeffs = np.zeros(len(frag_order))  # number of fragments
            ...
            frag_grain_mins = np.repeat(None, len(frag_order))
            frag_grain_mins[self.er_frag_indices] = frag_grain_mins_free    
            frag_grain_maxs = np.repeat(None, len(frag_order))
            ...
            # print(frag_types, frag_heights, frag_numbers, frag_masses, frag_ab_coeffs, frag_gammas, frag_er_coeffs, 
            #       frag_grain_mins, frag_grain_maxs)
...
ER_FRAG_INDICES = [0, 1, 2, 4, 5, 6, 7]# , 9]#, 2, 3, 4]  # free fragments that have erosion coefficients
MASS_FRAG_INDICES = [0, 1, 2, 3, 4, 5, 6, 7]  # fragments that have masses (i.e. everything except A)
...
metsim_obj = MetSimObj(traj_path=event_path + '.pickle', 
                       ...
                       mass_frag_indices = MASS_FRAG_INDICES
                      )
...
def unflatten_list(lst, num_global_params=1, num_frag_params=6, er_frag_indices=ER_FRAG_INDICES, mass_frag_indices=MASS_FRAG_INDICES, free_frag_indices=FREE_FRAG_INDICES):
    """
    ...
    sizes = list(np.repeat('k', num_frag_params - 5))  # CHANGE THIS IF STRUCTURE CHANGES
    sizes.insert(0, int(num_global_params))  # CHANGE THIS IF STRUCTURE CHANGES
    sizes[1:1] = [int(len(list(set(free_frag_indices) - set(mass_frag_indices))))]
    sizes[2:2] = [int(len(mass_frag_indices))]
    sizes[3:3] = [int(len(er_frag_indices))] * 3  # CHANGE THESE INDICES IF STRUCTURE CHANGES
    # print(sizes)
...
frag_gammas = tuple((0.1, 1.5) for _ in range(fragmentation_count - len(MASS_FRAG_INDICES)))
frag_mass_percents = tuple((0., 100.) for _ in range(len(MASS_FRAG_INDICES)))
...
frag_grain_mins = tuple([(1e-6, 1e2) for entry in [metsim_obj.fragmentation_entries[i] for i in ER_FRAG_INDICES]])
# Frag grain maxs
frag_grain_maxs = tuple([(1e-5, 1e3) for entry in [metsim_obj.fragmentation_entries[i] for i in ER_FRAG_INDICES]])
# Frag heights
...
result = frag_gammas + frag_mass_percents + frag_erosion_coeffs + frag_grain_mins + frag_grain_maxs + frag_heights
...
# mass bounds
bounds[0] = (1.e2, 1.e5)
bounds[1 + fragmentation_count - len(MASS_FRAG_INDICES):len(frag_mass_percents) + 2] = np.repeat([(0., 100.)], len(frag_mass_percents), axis=0)
# Height bounds:
# CHANGE THIS DEPENDING ON HEIGHT
height_bounds = 850.  # permissible values plus or minus the starting fragmentation height
bounds[-fragmentation_count:-1] = np.array((initial_guess[-fragmentation_count:-1] - height_bounds, 
                                                         initial_guess[-fragmentation_count:-1] + height_bounds)).T
bounds[-1] = (20000., 80000.)
...
# PRIOR
def prior(cube, tup_bounds=bounds, n_frags=fragmentation_count, er_frags=len(ER_FRAG_INDICES), mass_frags = len(MASS_FRAG_INDICES), free_frags=len(FREE_FRAG_INDICES)):
    """
    Transform the unit cube to a uniform prior
    """
    # bounds
    gamma_frags = free_frags - mass_frags
    bounds = np.array(tup_bounds)
    mass_bnds = bounds[0]
    frag_gamma_bnds = bounds[1:gamma_frags + 1]
    frag_mass_pct_bnds = bounds[gamma_frags + 1:gamma_frags + mass_frags + 1]
    frag_er_coeff_bnds = bounds[gamma_frags + mass_frags + 1: gamma_frags + mass_frags + er_frags + 1]
    frag_grain_min_bnds = bounds[gamma_frags + mass_frags + er_frags + 1: gamma_frags + mass_frags + 2 * er_frags + 1]
    frag_grain_max_bnds = bounds[gamma_frags + mass_frags + 2 * er_frags + 1: gamma_frags + mass_frags + 3 * er_frags + 1]
    frag_height_bnds = bounds[gamma_frags + mass_frags + 3 * er_frags + 1: gamma_frags + mass_frags + 3 * er_frags + n_frags + 1]
    # particle params
    # mass
    cube[0] = cube[0] * (mass_bnds[1] - mass_bnds[0]) + mass_bnds[0] # transform [0, 1] unit cube to [mass_lower, mass_uper]
    # gammas
    cube[1:gamma_frags + 1] = cube[1:gamma_frags + 1] * (frag_gamma_bnds[:, 1] - frag_gamma_bnds[:, 0]) + frag_gamma_bnds[:, 0]  # transform gammas
    # mass pct
    cube[gamma_frags + 1:gamma_frags + mass_frags + 1] = cube[gamma_frags + 1:gamma_frags + mass_frags + 1] * (frag_mass_pct_bnds[:, 1] - frag_mass_pct_bnds[:, 0]) + frag_mass_pct_bnds[:, 0]  # transform [0, 1] unit cube
    # erosion coeff
    cube[gamma_frags + mass_frags + 1: gamma_frags + mass_frags + er_frags + 1] = cube[gamma_frags + mass_frags + 1: gamma_frags + mass_frags + er_frags + 1] * (frag_er_coeff_bnds[:, 1] - frag_er_coeff_bnds[:, 0]) + frag_er_coeff_bnds[:, 0]
    # grain mins
    cube[gamma_frags + mass_frags + 1 * er_frags + 1: gamma_frags + mass_frags + 2 * er_frags + 1] = cube[gamma_frags + mass_frags + 1 * er_frags + 1: gamma_frags + mass_frags + 2 * er_frags + 1] * (frag_grain_min_bnds[:, 1] - frag_grain_min_bnds[:, 0]) + frag_grain_min_bnds[:, 0]
    # grain maxs
    cube[gamma_frags + mass_frags + 2 * er_frags + 1: gamma_frags + mass_frags + 3 * er_frags + 1] = cube[gamma_frags + mass_frags + 2 * er_frags + 1: gamma_frags + mass_frags + 3 * er_frags + 1] * (frag_grain_max_bnds[:, 1] - cube[n_frags + er_frags + 1: n_frags + 2 * er_frags + 1]) + cube[n_frags + er_frags + 1: n_frags + 2 * er_frags + 1]
    # heights
    cube[gamma_frags + mass_frags + 3 * er_frags + 1: gamma_frags + mass_frags + 3 * er_frags + n_frags + 1] = cube[gamma_frags + mass_frags + 3 * er_frags + 1: gamma_frags + mass_frags + 3 * er_frags + n_frags + 1] * (frag_height_bnds[:, 1] - frag_height_bnds[:, 0]) + frag_height_bnds[:, 0]
    # print(cube)
    return cube
...
def log_likelihood(particle, metsim_obj=metsim_obj, timeout=15):
    ...
    """
    # observed LC intensity, this doesn't change
    obs_lc_intensity = 3030 * (10 ** (np.concatenate(list(metsim_obj.lc_data.abs_mag_data.values()))/(-2.5)))
    ...
    simulated_lc_intensity = np.interp(np.concatenate(list(metsim_obj.lc_data.height_data.values())), 
                                       np.flip(simulation_results.leading_frag_height_arr[:first_increasing_height_arr]), 
                                       np.flip((3030 * (10 ** (simulation_results.abs_magnitude/(-2.5))))[:first_increasing_height_arr]))
...
# Run NS:
import dynesty
filename = './dynesty_saves/novo_mesto_unrestricted_sorted_gamma.save'  # CHANGE THIS

n_threads = multiprocessing.cpu_count() - 1
# n_threads = 32
# n_threads = 48
# n_threads = 50
timeout = 20  # number of seconds to timeout
